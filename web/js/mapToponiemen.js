...
function demGetToponiemen(gem,top){
    
    var keyValueList = new Array;

    if (top == "") top = 'Alle';
    
    targetUrl="http://"+websiteIP+websitePath+"/CRUDScripts/zoekPercelenVanToponiemen.script.php";
    $('#map').html('');
    if (top.includes('Alle')) {
        
        keyValueList[0] = 'gemeente##'+gem;
        getMapTop(keyValueList,gem);
    } else {
            
    argumenten = '?gemeente='+gem+'&toponiemen='+top;
...
function getMapTop(keyValueList,gemeente)
{
    var mywindow = null;
